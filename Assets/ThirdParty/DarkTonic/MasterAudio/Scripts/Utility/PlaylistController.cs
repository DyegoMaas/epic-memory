...
public class PlaylistController : MonoBehaviour
{
    ...
    void Update() {
        ...
        if (IsAutoAdvance)
        {
            if (!activeAudio.loop && activeAudio.clip != null)
            {
                ...
                if (currentClipTime < clipFadeStartTime)
                {
					if (isShuffle) {
						...
						if (!loopPlaylist && (currentSequentialClipIndex >= currentPlaylist.MusicSettings.Count || currentPlaylist.MusicSettings.Count == 1)) {
							FadeOutPlaylist();
							return;
						}
					}
			...
			// still need to release resources if auto-advance is off
			if (!activeAudio.loop && !activeAudio.isPlaying && activeAudio.clip != null) {
	...
	private void FadeOutPlaylist() {
		...
		FadeToVolume(0f, MasterAudio.Instance.CrossFadeTime, delegate() {
			StopPlaylist();
		});
	}
